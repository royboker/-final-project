"""
Create train/val/test splits for the 15K driver's license dataset.
Uses GroupShuffleSplit on original_id to prevent data leakage.

Same split strategy as the 9,996 dataset: 80/10/10.

Usage: python notebooks/drivers_license_forgery/production/prepare_splits.py
"""
import os
import pandas as pd
...
DATASET_CSV = os.path.join(PROJECT_ROOT, "datasets/drivers_license_15k/dataset.csv")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "notebooks/drivers_license_forgery/production/data")
...
def main():
    ...
    # Also copy the full dataset.csv for reference
