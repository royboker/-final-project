...
This folder contains the training notebooks, models, and evaluation scripts for a **2-phase forgery detection pipeline** on ID card documents from 3 countries (RUS, SVK, NV).
...
The production pipeline trained on **20,000 images** — 10,000 real and 10,000 fake ID cards from 3 countries (RUS, SVK, NV), with ~3,333 subjects per country.
...
- **RUS** (Russia), **SVK** (Slovakia), **NV** (Nevada)
- Each subject has a real photo + morphed version + replaced version
- The 20K dataset is stored at `datasets/id_card_20k/` and was created by `src/data/create_id_card_20k_dataset.py`
...
- **Perfectly balanced** — 50/50 real/fake in every split, equal country distribution (RUS/SVK/NV)
