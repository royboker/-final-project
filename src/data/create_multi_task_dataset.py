...
OUTPUT_DIR = 'datasets/drivers_license_multi_task'  # New name for multi-task dataset
    ...
    for country in TARGET_COUNTRIES:
        ...
        for i, pid in enumerate(tqdm(selected_ids)):
            ...
            csv_rows.append({
                ...
                'label': 0,  # Keep for backward compatibility
                'label_name': 'real',  # Keep for backward compatibility
                'is_fake': 0,  # NEW: Binary label for Task 1
                'fraud_type': '',  # NEW: Empty for Real images (Task 2)
                'original_id': unique_subject_id,
                'source_country': country,
                'original_filename': fname
            })
            
            # --- 2. Fake Image (One per subject, split 50/50) ---
            if i < mid_point:
                # Morphing
            ...
            csv_rows.append({
                ...
                'label': lbl,  # Keep for backward compatibility
                'label_name': lbl_name,  # Keep for backward compatibility
                'is_fake': 1,  # NEW: Binary label for Task 1
                'fraud_type': fraud_type,  # NEW: Fraud type for Task 2
                'original_id': unique_subject_id,
                'source_country': country,
                'original_filename': fname
            })
